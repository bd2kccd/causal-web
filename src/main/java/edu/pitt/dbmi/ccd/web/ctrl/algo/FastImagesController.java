...
public class FastImagesController implements ViewPath {
    ...
    public String showGesView(@ModelAttribute("appUser") final AppUser appUser, final Model model) {
        ...
        Map<String, String> map = algorithmService.getUserRunnableData(appUser.getUsername());//algorithmService.getUserRunnableData(".*txt", appUser.getUsername());
        if (map.isEmpty()) {
            info.setDataset("");
        } else {
    ...
    public String runGes(
        ...
        jobRequest.setAlgorithmName("fastImages");
